#!/usr/bin/env python3
"""Independent mathematical reference model for the SEC 2 sect233k1 curve.

This module deliberately contains no RTL lookup tables, path masks, or HDEC
implementation details.  It is used only to generate and check VV30 vectors.
"""
...
def gf_mul(left: int, right: int) -> int:
    """Polynomial multiplication followed by independent bitwise reduction."""
    left &= FIELD_MASK
    right &= FIELD_MASK
    product = 0
    while right:
...
def gf_inv(value: int) -> int:
    """Extended Euclidean inversion in GF(2)[x]."""
    value &= FIELD_MASK
    if value == 0:
...
def is_on_curve(point: AffinePoint | None) -> bool:
    ...
    # Binary Weierstrass form: y^2 + xy = x^3 + ax^2 + b.
...
def scalar_mul(
    ...
    """Variable-time double-and-add reference multiplication."""
    if scalar < 0:
...
def self_test() -> None:
    ...
    if scalar_mul(0) is not INFINITY:
        raise AssertionError("0*G must be the point at infinity")
    if scalar_mul(1) != BASE_POINT:
        raise AssertionError("1*G must equal G")
    if scalar_mul(3) != expected_3g:
        raise AssertionError("independent model does not reproduce the RTL 3G vector")
    if scalar_mul(GROUP_ORDER) is not INFINITY:
        raise AssertionError("n*G must be the point at infinity")
    for scalar in (2, 5, 17, 0x12345):
        point = scalar_mul(scalar)
        if not is_on_curve(point):
            raise AssertionError(f"{scalar}*G is not on the curve")
        if point_add(point, point_neg(point)) is not INFINITY:
            raise AssertionError(f"P + (-P) failed for scalar {scalar}")
        if point_add(point, BASE_POINT) != scalar_mul(scalar + 1):
            raise AssertionError(f"group addition failed for scalar {scalar}")


if __name__ == "__main__":
    self_test()
    print("[VV30:k233_reference] PASS")
