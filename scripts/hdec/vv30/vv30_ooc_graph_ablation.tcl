# Vivado 2024.2 matched OOC synthesis for the VV30 PMUL graph ablation.
#
# Usage:
#   vivado -mode batch -source vv30_ooc_graph_ablation.tcl -tclargs \
#     <rtl_repo_root> <out_root> <period_ns> <dbl_frobenius:0|1> \
#     <add_z_forward:0|1> <label>

if {$argc != 6} {
    error "Expected: <rtl_repo_root> <out_root> <period_ns> <dbl_frobenius> <add_z_forward> <label>"
}

set repo_root      [file normalize [lindex $argv 0]]
set out_root       [file normalize [lindex $argv 1]]
set period_ns      [lindex $argv 2]
set dbl_frobenius  [lindex $argv 3]
set add_z_forward  [lindex $argv 4]
set run_label      [lindex $argv 5]

foreach value [list $dbl_frobenius $add_z_forward] {
    if {($value ne "0") && ($value ne "1")} {
        error "graph-ablation controls must be 0 or 1"
    }
}
