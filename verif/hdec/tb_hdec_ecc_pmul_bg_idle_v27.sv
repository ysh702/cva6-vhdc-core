module tb_hdec_ecc_pmul_bg_idle_v27;
    import hdec_pkg::*;
    ...
    initial begin
        ...
        bg_wait_cycles = 0;
        while (!dut.ecc_job_done_q && (bg_wait_cycles < 1000000)) begin
            @(posedge clk_i);
            bg_wait_cycles++;
        end
        issue(HDEC_ECC_STATUS, '0, status);
        if ((status[1:0] !== STATUS_OK) || (status[3] !== 1'b1) || (status[2] !== 1'b0)) begin
            error_count++;
            $error("Background idle PMUL not done status=0x%016h", status);
        end
        $display("PMUL_BG_IDLE_WALL_CYCLES=%0d", bg_wait_cycles);
        $display("PMUL_BG_IDLE_STATUS_LOW16=%0d", status[31:16]);

        check_row("PMUL_BG_IDLE_3G_X", 6'd4, expected_x_3g);
        check_row("PMUL_BG_IDLE_3G_Y", 6'd5, expected_y_3g);

        if (error_count == 0)
            $display("[HDEC_ECC_PMUL_BG_IDLE_V27] PASS");
        else
            $fatal(1, "[HDEC_ECC_PMUL_BG_IDLE_V27] FAIL errors=%0d", error_count);
        $finish;
    end
