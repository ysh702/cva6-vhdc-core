module tb_hdec_vv24_native_xor1;
    import hdec_pkg::*;
    ...
    function automatic logic [30:0] diag_ref(input logic [7:0][31:0] matrix);
        ...
        begin
            ...
            for (int flat_idx = 0; flat_idx < 256; flat_idx++)
                product[hdec_vv24_matrix_diag(flat_idx)] ^=
    ...
    task automatic drive_pop_parity;
        begin
            diag_pop_parity = '0;
            for (int diag_idx = 0; diag_idx < 31; diag_idx++) begin
                if (hdec_vv24_pop_owner(diag_idx)) begin
                    for (int term_idx = 0;
                         term_idx < hdec_vv24_diag_len(diag_idx);
                         term_idx++) begin
                        diag_pop_parity[diag_idx] ^=
                            matrix_product[
                                hdec_vv24_matrix_flat(diag_idx, term_idx) / 32
                            ][
                                hdec_vv24_matrix_flat(diag_idx, term_idx) % 32
                            ];
                    end
        ...
        begin
            if (diag_mode) begin
                if (diag16_product !== diag_ref(matrix_product))
                    $fatal(1,
                        "[VV24_NATIVE_XOR1] diag mismatch cycle=%0d got=%h exp=%h",
                ...
                if (legacy_accum !== legacy_accum_ref(
                    ...
                    $fatal(1, "[VV24_NATIVE_XOR1] legacy accum mismatch cycle=%0d",
                           cycle_idx);
                if (legacy_lowxor_a !== (legacy_leaf_a ^ legacy_leaf_xor_a))
                    $fatal(1, "[VV24_NATIVE_XOR1] legacy A mismatch cycle=%0d",
                           cycle_idx);
                if (legacy_lowxor_b !== (legacy_leaf_b ^ legacy_leaf_xor_b))
                    $fatal(1, "[VV24_NATIVE_XOR1] legacy B mismatch cycle=%0d",
                           cycle_idx);
            end
        end
    endtask
    ...
    initial begin
        ...
        $display("[VV24_NATIVE_XOR1] PASS legacy_cycles=2048 diag_cycles=2048 native_nodes=224 diag_nodes=87");
        $finish;
    end
endmodule
