...
module tb_vv35_schedule_gate;
    ...
    // Portable activity path for ASIC simulators. XSim uses the dedicated
    // Tcl SAIF recorder, while VCS/other simulators can pass
    // +ACTIVITY_VCD=<path>. Only the metric_active interval is dumped.
    initial begin : portable_vcd_capture
        string activity_vcd;
        if ($value$plusargs("ACTIVITY_VCD=%s", activity_vcd)) begin
            $dumpfile(activity_vcd);
            $dumpvars(0, dut);
            $dumpoff;
            wait (metric_active === 1'b1);
            $display("[VV35:ACTIVITY_VCD] START file=%0s", activity_vcd);
            $dumpon;
            wait (metric_active === 1'b0);
            $dumpoff;
            $dumpflush;
            $display("[VV35:ACTIVITY_VCD] STOP file=%0s", activity_vcd);
        end
    end
