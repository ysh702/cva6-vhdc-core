module tb_hdec_ecc_pmul_bg_v27;
    import hdec_pkg::*;
    ...
    task automatic check_equal64(input string label,
        ...
        begin
            if (got !== expected) begin
                error_count++;
                $error("%s mismatch got=0x%016h expected=0x%016h", label, got, expected);
            end
        end
    endtask

    task automatic poll_pmul_done(output logic [63:0] status);
        bit done;
        begin
            done = 1'b0;
            for (int poll = 0; poll < 2000 && !done; poll++) begin
            ...
            if (!done) begin
                error_count++;
                $fatal(1, "Timeout waiting for background PMUL status=0x%016h", status);
            end
        end
    endtask
    ...
    initial begin
        ...
        issue(HDEC_HBIND, hbind_operand(4'd4, 4'd2, 4'd3), status);
        ...
        check_row("PMUL_BG_3G_X", 6'd4, expected_x_3g);
        check_row("PMUL_BG_3G_Y", 6'd5, expected_y_3g);

        if (error_count == 0)
            $display("[HDEC_ECC_PMUL_BG_V27] PASS");
        else
            $fatal(1, "[HDEC_ECC_PMUL_BG_V27] FAIL errors=%0d", error_count);
        $finish;
    end
endmodule
