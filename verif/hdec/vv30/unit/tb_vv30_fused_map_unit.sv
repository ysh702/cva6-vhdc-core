module tb_vv30_fused_map_unit;
    ...
    function automatic logic [255:0] independent_fused_map(
        ...
        begin
            ...
            case (group_idx)
                2'd0: local_product = diagonal64 ^ (diagonal64 << 16);
            ...
            reduced = vv30_gf233_reduce512(raw_polynomial);
            independent_fused_map = {23'd0, reduced};
        end
    endfunction

    initial begin
        logic [30:0] diagonal;
        logic [63:0] diagonal_product;
        logic [6:0] path_mask;
            ...
            for (int sub_idx = 0; sub_idx < 3; sub_idx++) begin
                for (int group_idx = 0; group_idx < 3; group_idx++) begin
                    for (int basis = 0; basis < 31; basis++) begin
                        diagonal = '0;
                        diagonal[basis] = 1'b1;
                        expected = independent_fused_map(
                            4'(path), 2'(sub_idx), 2'(group_idx), diagonal
                        );

                        diagonal_product = dut.ecc_diag16_karatsuba_token(
                            3'(group_idx), {1'b0, diagonal}
                        );
                        path_mask = dut.ecc_kpd64_leaf_offset_mask(4'(path));
                        got_packet = dut.ecc_kpd64_subproduct_reduce_packet(
                            path_mask, 2'(sub_idx), diagonal_product
                        );
                        ...
                        if (got !== expected)
                            $fatal(1,
                                "[VV30:fused_map_unit] path=%0h sub=%0d group=%0d basis=%0d got=%064h expected=%064h",
                                path, sub_idx, group_idx, basis, got, expected);
                        if (got[255:233] !== 23'd0)
                            $fatal(1,
                                "[VV30:fused_map_unit] nonzero high padding");
                        observed++;
                    end
                end
            end
        ...
        $display("[VV30:COVERAGE] test=fused_map_unit observed=%0d expected=%0d",
                 observed, VV30_FUSED_MAP_COVERAGE);
        if (observed != VV30_FUSED_MAP_COVERAGE)
            $fatal(1, "[VV30:fused_map_unit] incomplete coverage");
        $display("[VV30:fused_map_unit] PASS");
        $finish;
    end
endmodule
