module tb_hdec_ecc_pmul_v18;
    ...
    initial begin
        ...
        issue(HDEC_ECC_STATUS, ecc_pmul_operand(6'd4, 6'd0, 6'd2), status);
        if (status[1:0] !== STATUS_OK) begin
            error_count++;
            $error("PMUL returned bad status 0x%016h", status);
        end
        $display("PMUL_CYCLES=%0d", status[31:16]);

        check_row("PMUL_3G_X", 6'd4, expected_x_3g);
        check_row("PMUL_3G_Y", 6'd5, expected_y_3g);

        if (error_count == 0)
            $display("[HDEC_ECC_PMUL_V18] PASS");
        else
            $fatal(1, "[HDEC_ECC_PMUL_V18] FAIL errors=%0d", error_count);
        $finish;
    end
endmodule
