...
module hdec_top import hdec_pkg::*; import hdec_resource_pkg::*; (
    ...
    typedef enum logic [2:0] { S_IDLE, S_EXEC, S_RD_WAIT, S_RESULT } st_t; st_t st_q,st_n;
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if(!rst_ni)begin st_q<=S_IDLE;res_q<='0;op_q<=HDEC_VWR64;a_q<='0;b_q<='0;bk_q<='0;vaddr_bank_q<='0;vaddr_idx_q<='0;end
        else begin st_q<=st_n;res_q<=res_n;op_q<=op_n;a_q<=a_n;b_q<=b_n;bk_q<=bk_n;vaddr_bank_q<=vaddr_bank_n;vaddr_idx_q<=vaddr_idx_n;end
    end
endmodule
