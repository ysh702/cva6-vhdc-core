...
module hdec_top import hdec_pkg::*; import hdec_resource_pkg::*; (
    ...
    logic [11:0] hmatch_budget_q,hmatch_budget_n;
    logic [11:0] budget_next;
    logic        update_best;
    ...
    // ── Lane compute wires ──────────────────────────────────────────────────
    logic [LANE_NUM-1:0]                 uop_lane_bool_valid; // uop path XOR enable
    ...
    always_comb begin
        ...
        hmatch_budget_n=hmatch_budget_q; budget_next='0; update_best=1'b0;
...
`ifndef SYNTHESIS
            assert ($onehot0({
                    ...
                    uop_p2_q.use_shift,
                    uop_p2_q.use_clip,
                    (uop_p2_q.use_xor && !uop_p2_q.use_popcount)
                }))
...
`endif
            ...
            if (uop_p2_q.use_clip)
                lane_clip_valid = '1;
            if (uop_p2_q.valid && uop_p2_q.use_popcount) begin
                lane_popcnt_n[0] = lane_popcount_count[0];
                lane_popcnt_n[1] = lane_popcount_count[1];
                lane_popcnt_n[2] = lane_popcount_count[2];
                lane_popcnt_n[3] = lane_popcount_count[3];
            end else if (uop_p2_q.valid && uop_p2_q.use_counter)
            ...
            end else if (uop_p2_q.valid && uop_p2_q.use_xor)
        ...
        S_UOP_P3_GLOBAL: begin
            ...
            UOP_HSIM_CHUNK, UOP_HMATCH_CHUNK: begin
                ...
                if (uop_p3_q.op_type == UOP_HMATCH_CHUNK) begin
                    budget_next     = hmatch_budget_q - {1'b0, group_dist};
                    hmatch_budget_n = budget_next;
                    update_best     = !budget_next[11] && (budget_next[10:0] != 11'd0);
                end
                if (uop_p3_q.chunk_idx == 2'd3) begin
                    if (uop_p3_q.op_type == UOP_HMATCH_CHUNK) begin
                        ...
                        if (uop_p3_q.is_last_class) begin
                            ...
                            hsim_total_n        = '0;
                            hmatch_budget_n     = {1'b0, hmatch_best_dist_n};
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if(!rst_ni)begin
            ...
            hmatch_num_q<='0;hmatch_best_idx_q<='0;hmatch_class_slot_q<='0;hmatch_best_dist_q<='0;hmatch_budget_q<=12'd0;
            ...
            hmatch_num_q<=hmatch_num_n;hmatch_best_idx_q<=hmatch_best_idx_n;hmatch_class_slot_q<=hmatch_class_slot_n;hmatch_best_dist_q<=hmatch_best_dist_n;hmatch_budget_q<=hmatch_budget_n;
