...
module hdec_top import hdec_pkg::*; import hdec_resource_pkg::*; #(
    ...
    typedef enum logic [6:0] {
        ...
        S_ECC_DIAG_ISSUE, S_ECC_DIAG_WAIT,
        S_ECC_LEAF_FOLD,
    ...
    logic [63:0]          ecc_leaf_a_q,  ecc_leaf_a_n;
    logic [63:0]          ecc_leaf_b_q,  ecc_leaf_b_n;
    logic [127:0]         ecc_leaf_prod_q, ecc_leaf_prod_n;
    ...
    logic [6:0]           ecc_leaf_offset_mask;
    ...
    typedef enum logic [1:0] {
        ECC_DIAG_BG_IDLE  = 2'd0,
        ECC_DIAG_BG_ISSUE = 2'd1,
    ...
    assign ecc_leaf_offset_mask = ecc_kpd64_leaf_offset_mask(ecc_leaf_path_q);
    ...
    assign ecc_product_even_contrib = ecc_kpd64_fold_word_contrib(ecc_leaf_offset_mask, {ecc_fold_word_q, 1'b0}, ecc_leaf_prod_q);
    assign ecc_product_odd_contrib  = ecc_kpd64_fold_word_contrib(ecc_leaf_offset_mask, {ecc_fold_word_q, 1'b1}, ecc_leaf_prod_q);
    ...
    assign ecc_diag_next_even_slot = ecc_diag_slot_q + 3'd1;
    assign ecc_diag_pair_last = (ecc_diag_slot_q == 3'd7);
    assign ecc_diag16_parity[0]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd0);
    assign ecc_diag16_parity[1]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd1);
    assign ecc_diag16_parity[2]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd2);
    assign ecc_diag16_parity[3]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd3);
    assign ecc_diag16_parity[4]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd4);
    assign ecc_diag16_parity[5]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd5);
    assign ecc_diag16_parity[6]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd6);
    assign ecc_diag16_parity[7]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd7);
    assign ecc_diag16_parity[8]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd8);
    assign ecc_diag16_parity[9]  = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd9);
    assign ecc_diag16_parity[10] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd10);
    assign ecc_diag16_parity[11] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd11);
    assign ecc_diag16_parity[12] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd12);
    assign ecc_diag16_parity[13] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd13);
    assign ecc_diag16_parity[14] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd14);
    assign ecc_diag16_parity[15] = ^ecc_diag64_partial(ecc_leaf_a_q, ecc_leaf_b_q, {1'b0, ecc_diag_slot_q, 4'b0000} + 8'd15);
    ...
    function automatic logic [63:0] ecc_limb64(input logic [255:0] value, input logic [1:0] limb);
        unique case (limb)
            2'd0: ecc_limb64 = value[63:0];
            2'd1: ecc_limb64 = value[127:64];
            2'd2: ecc_limb64 = value[191:128];
            default: ecc_limb64 = value[255:192];
        endcase
    endfunction

    function automatic logic [5:0] ecc_kpd64_path_inc(input logic [5:0] path);
        logic [1:0] d1, d0;
        begin
            d1 = path[3:2];
            d0 = path[1:0];
            ...
            ecc_kpd64_path_inc = {2'b00, d1, d0};
        end
    endfunction
    ...
    function automatic logic [63:0] ecc_diag64_partial(
        input logic [63:0] a_word,
        input logic [63:0] b_word,
        input logic [7:0]  diag_idx
    );
        logic [63:0] partial;
        int signed b_idx;
        begin
            partial = '0;
            for (int signed i = 0; i < 64; i++) begin
                b_idx = int'(diag_idx) - i;
                if ((b_idx >= 0) && (b_idx < 64))
                    partial[i] = a_word[i] & b_word[b_idx];
            end
            ecc_diag64_partial = partial;
        end
    endfunction
    ...
    function automatic logic [127:0] ecc_kpd64_leaf_store_pack16(
        ...
        begin
            ...
            unique case (group_idx)
                3'd0: updated[15:0]    = parity;
                3'd1: updated[31:16]   = parity;
                3'd2: updated[47:32]   = parity;
                3'd3: updated[63:48]   = parity;
                3'd4: updated[79:64]   = parity;
                3'd5: updated[95:80]   = parity;
                3'd6: updated[111:96]  = parity;
                3'd7: begin
                    updated[126:112] = parity[14:0];
                    updated[127]     = 1'b0;
                end
                default: updated = leaf_product;
            endcase
            ecc_kpd64_leaf_store_pack16 = updated;
        end
    endfunction

    function automatic logic [6:0] ecc_kpd64_leaf_offset_mask(input logic [5:0] path);
        unique case (path[3:0])
    ...
    always_comb begin
        ...
        S_HSPREAD_HI_WRITE: begin
            vrf_req.we = '1;
            vrf_req.wa=hperm_dst_base_q + 6'd1;
        ...
        S_ECC_LOAD_A: begin
            ecc_leaf_a_n = ecc_kpd64_leaf_word({vrf_rd[3], vrf_rd[2], vrf_rd[1], vrf_rd[0]}, ecc_leaf_path_q);
        ...
        S_ECC_LOAD_B: begin
            ecc_leaf_b_n = ecc_kpd64_leaf_word({vrf_rd[3], vrf_rd[2], vrf_rd[1], vrf_rd[0]}, ecc_leaf_path_q);
            ecc_leaf_prod_n = '0;
            ecc_leaf_path_n = '0;
        ...
        S_ECC_LEAF_FOLD: begin
            ...
            if (!ecc_leaf_last) begin
                unique case (ecc_fold_word_q)
                    ...
                    2'd2: begin
                        ecc_leaf_a_n = ecc_kpd64_leaf_word({vrf_rd[3], vrf_rd[2], vrf_rd[1], vrf_rd[0]}, ecc_next_leaf_path);
                    end
                    default: begin
                        ecc_leaf_b_n = ecc_kpd64_leaf_word({vrf_rd[3], vrf_rd[2], vrf_rd[1], vrf_rd[0]}, ecc_next_leaf_path);
                    end
                endcase
            end
        ...
        S_ECC_PMUL_STEP: begin
            ...
            ECC_PMUL_SUB_AFFINE: begin
                ...
                6'd1: begin
                    ...
                    ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0;
            ...
            ECC_PMUL_SUB_DBL: begin
                unique case (ecc_pmul_step_q)
                6'd0: begin hperm_dst_base_n=ECC_PMUL_T0; hperm_spread_n=1'b1; ecc_dst_n=ECC_PMUL_T0; ecc_src_a_n=ECC_PMUL_T0; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ecc_pmul_dbl_x; st_n=S_RD_WAIT; end
                6'd1: begin hperm_dst_base_n=ECC_PMUL_T1; hperm_spread_n=1'b1; ecc_dst_n=ECC_PMUL_T1; ecc_src_a_n=ECC_PMUL_T1; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_T0; st_n=S_RD_WAIT; end
                6'd2: begin hperm_dst_base_n=ECC_PMUL_T4; hperm_spread_n=1'b1; ecc_dst_n=ECC_PMUL_T4; ecc_src_a_n=ECC_PMUL_T4; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ecc_pmul_dbl_z; st_n=S_RD_WAIT; end
                6'd3: begin hperm_dst_base_n=ECC_PMUL_T5; hperm_spread_n=1'b1; ecc_dst_n=ECC_PMUL_T5; ecc_src_a_n=ECC_PMUL_T5; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_T4; st_n=S_RD_WAIT; end
                ...
                default: begin hperm_dst_base_n=ecc_pmul_dbl_z; hperm_spread_n=1'b1; ecc_dst_n=ecc_pmul_dbl_z; ecc_src_a_n=ecc_pmul_dbl_z; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_T2; st_n=S_RD_WAIT; end
                endcase
            end
            ECC_PMUL_SUB_ADD: begin
                unique case (ecc_pmul_step_q)
                6'd0: begin ecc_dst_n=ECC_PMUL_T0; ecc_src_a_n=ECC_PMUL_R0X; ecc_src_b_n=ECC_PMUL_R1Z; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_leaf_a_n='0; ecc_leaf_b_n='0; ecc_leaf_prod_n='0; ecc_leaf_path_n='0; ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_R0X; st_n=S_ECC_LOAD_A_WAIT; end
                6'd1: begin ecc_dst_n=ECC_PMUL_T1; ecc_src_a_n=ECC_PMUL_R1X; ecc_src_b_n=ECC_PMUL_R0Z; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_leaf_a_n='0; ecc_leaf_b_n='0; ecc_leaf_prod_n='0; ecc_leaf_path_n='0; ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_R1X; st_n=S_ECC_LOAD_A_WAIT; end
                ...
                6'd3: begin hperm_dst_base_n=ECC_PMUL_T5; hperm_spread_n=1'b1; ecc_dst_n=ECC_PMUL_T5; ecc_src_a_n=ECC_PMUL_T5; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_T4; st_n=S_RD_WAIT; end
                6'd4: begin ecc_dst_n=ECC_PMUL_T2; ecc_src_a_n=ecc_pmul_scalar_bit_q ? ECC_PMUL_R1X : ECC_PMUL_R0X; ecc_src_b_n=ecc_pmul_scalar_bit_q ? ECC_PMUL_R1Z : ECC_PMUL_R0Z; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_leaf_a_n='0; ecc_leaf_b_n='0; ecc_leaf_prod_n='0; ecc_leaf_path_n='0; ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ecc_pmul_scalar_bit_q ? ECC_PMUL_R1X : ECC_PMUL_R0X; st_n=S_ECC_LOAD_A_WAIT; end
                6'd5: begin ecc_dst_n=ECC_PMUL_T6; ecc_src_a_n=ECC_PMUL_T0; ecc_src_b_n=ECC_PMUL_T1; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_leaf_a_n='0; ecc_leaf_b_n='0; ecc_leaf_prod_n='0; ecc_leaf_path_n='0; ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ECC_PMUL_T0; st_n=S_ECC_LOAD_A_WAIT; end
                6'd6: begin ecc_dst_n=ECC_PMUL_T7; ecc_src_a_n=ecc_pmul_point_q; ecc_src_b_n=ECC_PMUL_T5; ecc_autoreduce_n=1'b1; ecc_mac_n=1'b0; ecc_sqr_repeat_n='0; ecc_leaf_a_n='0; ecc_leaf_b_n='0; ecc_leaf_prod_n='0; ecc_leaf_path_n='0; ecc_diag_slot_n='0; ecc_fold_word_n='0; ecc_pipe0_valid_n=1'b0; ecc_job_phase_n=ECC_PHASE_PMUL_FIELD; vrf_req.ra=ecc_pmul_point_q; st_n=S_ECC_LOAD_A_WAIT; end
                ...
                default: begin ecc_job_phase_n=ECC_PHASE_PMUL_COPY; ecc_job_copy_dst_n=ecc_pmul_add_out_z; vrf_req.ra=ECC_PMUL_T5; st_n=S_ECC_INV_COPY_WAIT; end
                endcase
            end
            default: begin
                st_n=S_ECC_PMUL_STEP_NEXT;
...
`endif
        ...
        // background sidecar execution.  It does not request shared resources.
        if (ecc_diag_issue_fire) begin
            if (ecc_pipe0_valid_q)
                ecc_leaf_prod_n = ecc_kpd64_leaf_store_pack16(ecc_leaf_prod_q, ecc_pipe0_diag_slot_q, ecc_diag16_pipe_q);
            ecc_pipe0_valid_n = 1'b1;
            ecc_pipe0_diag_slot_n = ecc_diag_slot_q;
            ecc_diag16_pipe_n = ecc_diag16_parity;
            if (ecc_diag_pair_last) begin
